...
| -------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ttl=           | seconds    | The number of seconds a segment will exist                                                                                                                                                                                                                                   |
...
| on_insert=     | True/False | Evaluate segment the moment data is inserted. This is useful if you have a need for real-time notification and are using triggers.                                                                                                                                           |
...
> :pushpin: Some segments can be fully derived by counting indexes and are very fast. Adding sequence or time constraints will result in the segment generator executing the script against the event set (OpenSet will automatically determine if this is required). Event based segments will be slower if they cover a large cross-section of people in the database. Once segments are created they can be used by name in the `segments=` parameter of other queries.
