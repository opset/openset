...
std::vector<int64_t> openset::mapping::PartitionMap::getNodesByPartitionId(int partitionId) const
{
	...
	auto part = part2node.find(partitionId);

	if (part != part2node.end())
...
bool openset::mapping::PartitionMap::isOwner(int partitionId, int64_t nodeId) const
{
	csLock lock(cs);
...
bool openset::mapping::PartitionMap::isMapped(int partitionId, int64_t nodeId) const
{
	csLock lock(cs);
...
openset::mapping::NodeState_e openset::mapping::PartitionMap::getState(int partitionId, int64_t nodeId) const
{
	csLock lock(cs);
...
void openset::mapping::PartitionMap::setOwner(int partitionId, int64_t nodeId)
{
	...
	std::pair<int, location_s>* pair;

	if (!part2node.count(partitionId))
		pair = part2node.emplace(partitionId, location_s{});
	else
		pair = part2node.get(partitionId);
...
void openset::mapping::PartitionMap::setState(int partitionId, int64_t nodeId, NodeState_e state)
{
	...
	// add the partition to the map if it isn't already there
	if (!part2node.count(partitionId))
		pair = part2node.emplace(partitionId, location_s{});
	else
		pair = part2node.get(partitionId);
...
bool openset::mapping::PartitionMap::swapState(int partitionId, int64_t oldOwner, int64_t newOwner) const
{
	csLock lock(cs);
	...
	auto nodePtr = part2node.get(partitionId)->second.getMapPtr();
...
void openset::mapping::PartitionMap::removeMap(int partitionId, int64_t nodeId, NodeState_e state) const
{
	csLock lock(cs);

	auto pair = part2node.get(partitionId);
