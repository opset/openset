            ...
            if (line.parts[0] == "agg" || line.parts[0] == "aggregate")
            {
                for (auto& c : capture)
                {
                    ...
                    if (!ColumnModifiers.count(c.parts[0]))
                        c.parts.insert(c.parts.begin(), isTableColumn(c.parts[0]) ? "count" : "var");
                    // if in table then value
            ...
            else
            {
                ...
                else if (isFloat(value))
                {
                    ...
                    opList.emplace_back(
                        ...
                        // dirty secret, doubles are actually ints multipled for 6 digits
                        debug);
                }
                    // is this a number (check for negative)
...
void QueryParser::tokenizeBlock(FirstPass& lines, const int blockId, BlockList& blockList,
                                MiddleBlockList& outputBlocks)
{
    ...
    const auto pushBlock = [&](MiddleOpList& lambdaBlock, const int newId,
    ...
    while (i < static_cast<int>(lines.size()))
    {
                ...
                "for"))
        {
            ...
            QueryParser::LineParts QueryParser::extractVariable(LineParts& conditions, const int startIdx,
                                                                int& reinsertIdx)
            {
                LineParts result;
            ...
            QueryParser::LineParts QueryParser::extractVariableReverse(
                LineParts& conditions, const int startIdx, int& reinsertIdx)
            {
                LineParts result;
                ...
                while (true)
                {
                    ...
                    do // I never DO these
                    {
                        ...
                        while (index < static_cast<int>(conditions.size()))
                        {
                            ...
                            if (conditions[0] == "for" ||
                                (static_cast<int>(conditions.size()) > 2 && conditions[0] == "continue" &&
                                    conditions[1] == "for"))
                            {
                                auto inPos = -1;
                                ...
                                conditions[index] == "COUNT_DISTINCT")
                            {
                                ...
                                if (aggregate.size() <= 2 ||
                                    (aggregate.size() > 2 && aggregate[2] != "if"))
                                {
                                ...
                                LineParts nextWhere = {"for", "agg_row", "in", "rows"};
                                ...
                                else if (agg == "MAX")
                                {
                                    ...
                                    newFunction.back().parts.back() =
                                        to_string(std::numeric_limits<int64_t>::min());
                                ...
                                else if (agg == "MIN")
                                {
                                    // overwite value that __agg_result (set to zero by default above)
                                    newFunction.back().parts.back() =
                                        to_string(std::numeric_limits<int64_t>::max());
                            ...
                            if (static_cast<int>(conditions.size()) > index + 2 &&
                                ...
                                ((!isRowIter && conditions[0] != "for") || (isRowIter && index > 4)))
                                //(conditions[0] != "continue" && conditions[1] != "for") &&
                                //(conditions[0] != "for"))
                            {
                                ...
                                isSplice(conditions, index))
                            {
                                ...
                                LineParts inParts;
                                inParts.insert(inParts.begin(), conditions.begin() + (index + 1),
                                               conditions.begin() + closingIdx);
                                ...
                                conditions.erase(conditions.begin() + startIdx,
                                                 conditions.begin() + (closingIdx + 1));
                                conditions.insert(conditions.begin() + startIdx, newSection.begin(),
                                                  newSection.end());
                                ...
                                TimeConstants.count(conditions[index + 1]))
                            {
                                ...
                                if (isNumeric(item))
                                {
                                    conditions.insert(conditions.begin() + index,
                                                      to_string(stol(item) * timeValue));
                                }
                                else
                                {
                                    // inject this
            ...
            int64_t QueryParser::parseHintConditions(const LineParts& conditions, HintOpList& opList, int64_t index,
                                                     const bool stopOnConditions)
            {
                            ...
                            switch (c.op)
                            {
                                ...
                                case OpCode_e::PSHTBLCOL:
                                    finCode.emplace_back(
                                        ...
                                        0,
                                        c.deferredStr.length() ? vars.userVars[c.deferredStr].index : NONE,
                                        c.debug);
                                    break;
            ...
            bool QueryParser::compileQuery(const char* query, Columns* columnsPtr, Macro_s& macros,
                                           ParamVars* templateVars)
            {
                try
                {
                    rawScript = query;
                ...
                catch (ParseFail_s& caught)
                {
                                        ...
                                        if (current.length() && current[0] != '#')
                                        {
                                            ...
                                            if (current[0] == '@')
                                            {
                                                ...
                                                if (sectionParts.size() >= 2)
                                                {
                                                    ...
                                                    for (auto idx = 2; idx < static_cast<int>(sectionParts.size());
                                                         ++idx)
                                                    {
                                                        ...
                                                        if (keyVal[0] == "ttl" || keyVal[0] == "refresh")
                                                            ...
                                                            flags["use_cached"] = (keyVal[1] == "True" || keyVal[1]
                                                                == "true");
                                                            ...
                                                            params[keyVal[0]] = (keyVal[1] == "True" || keyVal[1] ==
                                                                "true");
                                                        else
                    ...
                    string padding(const int64_t number, const int length, const bool left = true,
                                   const char filler = ' ')
                    {
                    ...
                    string openset::query::MacroDbg(Macro_s& macro)
                    {
                        ...
                        ss << "Aggregates:" << endl;
                        outSpacer();
                        ss << "AGGIDX | TBLIDX | AGG    | NAME                 | ALIAS                | NOTE" <<
                            endl;
                        outSpacer();
