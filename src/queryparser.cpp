...
int64_t QueryParser::expandTime(string value)
{
    ...
    {
        returnValue = stoll(value.substr(0, value.length() - 1)); // does the number end in s, m, or h
        switch (lastChar)
        {
        case 's':
            break;
...
QueryParser::LineParts QueryParser::breakLine(const string& text)
{
    ...
    while (c < end)
    {
        if (c[0] == '#')
            break;
        if ((c[0] == '!' && c[1] == '=') || (c[0] == '+' && c[1] == '=') || (c[0] == '-' && c[1] == '=') || (c[0] == '*'
        ...
        else if (c[0] == '=' || c[0] == '+' || c[0] == '-' || c[0] == '(' || c[0] == ')' || c[0] == '[' || c[0] == ']'
            || c[0] == '{' || c[0] == '}' || c[0] == ',' || c[0] == '*' || c[0] == '/')
        {
            push(part, parts);
            part += c[0];
...
QueryParser::FirstPass QueryParser::extractLines(const char* query)
{
    ...
    while (c < end)
    {
        ...
        case '\n':
        {
            ...
            current.text = trim(current.text, " "); // if this line isn't empty, and isn't a comment
            // store it
            if (current.text.length() && current.text[0] != '#')
            {
                ...
                do
                {
                    ...
                    if (templating)
                        for (auto k : *templating)
                        {
                            ...
                            {
                                changed = true; // trim out the variable and the brackets
                                current.text.erase(idx, search.length());
                                // insert without quotes if this is a column name
                                if (isTableColumn(k.second))
                                    current.text.insert(idx, k.second); // if this is a string we insert it with quotes
                ...
                while (changed); // break line into words
    ...
    for (auto i = 0; i < static_cast<int>(lines.size()); i++)
    {
        ...
        if (lines[i].indent > indent)
        {
            ...
            if (line.parts[0] == "agg" || line.parts[0] == "aggregate")
            {
                for (auto& c : capture)
                {
                    ...
                    if (!ColumnModifiers.count(c.parts[0]))
                        c.parts.insert(
                            c.parts.begin(),
                            isTableColumn(c.parts[0])
                                ? "count"
                                : "var"); // if in table then value
                    ...
                    if (c.parts[1] == "session")
                    {
                        ...
                        // distinct total for each match for each person, making it only really useful for
                        // counting sessions
                    ...
                    if (lambdaIdx != -1) // found a lambda? Then assign a lambda assignment
                    {
                        ...
                        copy(c.parts.begin() + index, c.parts.end(), back_inserter(lambdaCapture));
                    ...
                    if (forceDistinct && !isTableColumn(distinct) && modifier->second != Modifiers_e::var)
                        throw ParseFail_s {
                            errors::errorClass_e::parse,
                    ...
                    if (isTableColumn(distinct))
                        vars.tableVars.emplace(distinct, Variable_s { distinct, "grid" });
            ...
            else if (line.parts[0] == "def" || line.parts[0] == "@trigger") // cheat on trigger
            {
                line.parts[0] = "def";
                ++blockCounter;
                line.block = extractBlocks(indent + 1, capture, blockList); // find our block
            ...
            else // just push back our block
            {
                ...
                if (line.parts.size() >= 1 && (line.parts[0] == "if" || line.parts[0] == "elif" || line.parts[0] ==
                    "else"))
                {
                    for (auto& b : blockList)
                    {
    ...
    const string& stackOp)
{
    while (index < conditions.size())
    {
        ...
        if (conditions[index] == "(" || conditions[index] == "[")
        {
            index = parseConditions(conditions, opList, index + 1, debug, stopOnConditions, stackOp);
        } // is it a comparative
        else if (Operators.find(conditions[index]) != Operators.end())
        {
            const int newindex = parseConditions(conditions, opList, index + 1, debug, true);
            opList.emplace_back(Operators.find(conditions[index])->second, 0, debug);
            index = newindex;
        }
        else if (Math.find(conditions[index]) != Math.end())
        {
            const int newindex = parseConditions(conditions, opList, index + 1, debug, true, stackOp); // force the stop
            opList.emplace_back(Math.find(conditions[index])->second, 0, debug);
            index = newindex;
        }
        else if (LogicalOperators.find(conditions[index]) != LogicalOperators.end())
        {
            if (stopOnConditions)
            {
                index--;
            ...
            const int newIndex = parseConditions(conditions, opList, index + 1, debug, false, conditions[index]);
            // pass in last op for hinting
            // don't stop on conditional (and/or) here!
            opList.emplace_back(LogicalOperators.find(conditions[index])->second, 0, debug);
            index = newIndex;
            break;
        }
        else if (conditions[index] == "None")
        {
            opList.emplace_back(OpCode_e::PSHLITNUL, 0, debug);
            break;
        }
        else
        {
            ...
            else
            {
                ...
                if (isTableColumn(value))
                {
                    ...
                    opList.emplace_back(OpCode_e::PSHTBLCOL, value, debug);
                    if (!isVar(vars.tableVars, value))
                    {
                        vars.tableVars.emplace(value, Variable_s { value, "grid" });
                    }
                }
                else if (isVar(vars.columnVars, value))
                {
                    opList.emplace_back(OpCode_e::PSHRESCOL, value, debug);
                }
                else if (isString(value))
                {
                    opList.emplace_back(OpCode_e::PSHLITSTR, value, debug); // the index will be set on the final pass
                ...
                else if (isFloat(value))
                {
                    ...
                    opList.emplace_back(
                        OpCode_e::PSHLITFLT,
                        cast<int64_t>(dblValue * 1'000'000),
                        // dirty secret, doubles are actually ints multipled for 6 decimal digits
                        debug);
                } // is this a number (check for negative)
                else if (isNumeric(value))
                {
                    ...
                    opList.emplace_back(OpCode_e::PSHLITINT, intValue, debug);
                }
                else if (isTextual(value) || value[0] == '@')
                {
                    ...
                    if (value[0] == '@')
                    {
                        ...
                        isRef = true;
                    } // look ahead 1 for [ (for member of object)
                    if (conditions.size() > index + 1 && conditions[index + 1] == "[")
                    {
                        // is it a an variable used in a row iterator i.e. "for __VAR__ in rows:"
                        if (isRowObject(value))
                        {
                            ...
                            */ // this is a set row command
                            ...
                            // for the set row
                            opList.emplace_back(OpCode_e::PSHTBLCOL, tableVar, debug);
                            opList.back().deferredStr = value;
                            if (!isVar(vars.tableVars, tableVar))
                            {
                                vars.tableVars.emplace(tableVar, Variable_s { tableVar, "grid" });
                            }
                            index += 3;
                        }
                        else
                        {
                            // else it's a member accessor
                            vector<MiddleOpList> derefCaptures;
                            auto derefEndIdx = index + 1;
                            while (derefEndIdx < conditions.size() && conditions[derefEndIdx] == "[")
                            {
                                ...
                                derefEndIdx = parseConditions(
                                    ...
                                    derefEndIdx + 1,
                                    debug,
                                    false); // TODO - check for zero length
                                derefCaptures.emplace_back(derefOps);
                                ++derefEndIdx;
                            }
                            --derefEndIdx; // insert the deref ops in reverse (stack) order
                            for_each(
                                derefCaptures.rbegin(),
                                derefCaptures.rend(),
                                [&opList](auto item)
                                {
                                    opList.insert(
                            ...
                            opList.emplace_back(
                                isRef
                                    ...
                                    : OpCode_e::PSHUSROBJ,
                                value,
                                debug); // params contains the depth of the deref
                    ...
                    else
                    {
                        ...
                        */
                        if (value == "True")
                        {
                            opList.emplace_back(OpCode_e::PSHLITTRUE, value, debug);
                        }
                        else if (value == "False")
                        {
                            opList.emplace_back(OpCode_e::PSHLITFALSE, value, debug);
                        }
                        else
                        {
                            opList.emplace_back(
...
int64_t QueryParser::parseCall(LineParts& conditions, MiddleOpList& opList, int64_t index, Debug_s& debug)
{
    ...
    auto params       = 0;
    index++; // these are function calls that appear like variables, they are read-only
...
QueryParser::LineParts QueryParser::tokenizeBlockForLogic(
    LineParts& parts,
    int searchFrom,
    bool isConditional,
    bool isFunction)
{
    LineParts condition;
    auto logicIndex = search(parts, "if", 3);
    if (logicIndex != -1)
    {
        for (logicIndex = logicIndex + 1; logicIndex < static_cast<int>(parts.size()); ++logicIndex)
        {
            if (parts[logicIndex] == "__MARKER__")
                break;
            condition.push_back(parts[logicIndex]);
        } //--------------
        // Index Hinting 
        //--------------
        ...
        auto& hints = hintMap["_"]; // if there are process conditions get lambda
        // we don't index row conditions if they occur within if/elif/else blocks
        ...
        if (condition.size() && !isConditional && !isFunction)
        {
            if (hints.size())
...
void QueryParser::tokenizeBlock(
    FirstPass& lines,
    const int blockId,
    BlockList& blockList,
    MiddleBlockList& outputBlocks)
{
    MiddleOpList block;
    auto i               = 0;
    const auto pushBlock = [&](
        ...
        const int newId,
        const BlockType_e blockType = BlockType_e::code)
    {
    ...
    while (i < static_cast<int>(lines.size()))
    {
        if (lines[i].parts[0] == "@flags")
        {
            for (auto x = 1; x < static_cast<int>(lines[i].parts.size()); ++x)
            {
                ...
                if (lines[i].parts[x] == "ttl" && static_cast<int>(lines[i].parts.size()) > x + 2 && lines[i].parts[x +
                    1] == "=")
                {
        ...
        else if (lines[i].parts[0] == "else")
        {
            ...
            */ 
        else if (lines[i].parts[0] == "__row_for" || lines[i].parts[0] == "__row_for_count")
        {
            /* original format:
             *    for <rowVar> in rows:
             * modified format:
             *    __row_for <rowVar> __rows:
             */
            const auto isCount       = lines[i].parts[0] == "__row_for_count";
            const auto maxIterations = isCount ? stoll(lines[i].parts[1]) : 99'999'999;
            const auto rowVar        = lines[i].parts[isCount ? 2 : 1];
            const auto reverseSearch = lines[i].parts[isCount? 3 : 2] == "__rev_rows";

            // look for `if` and generate index hints
            auto conditions = tokenizeBlockForLogic(lines[i].parts, 3, lines[i].isConditional, lines[i].isFunction);
            ...
            int64_t lambdaId = -1; 
            // if there was an "if" in this iterator
            ...
            if (!conditions.empty())
            {
                ...
                MiddleOpList lambdaBlock;
                parseConditions(conditions, lambdaBlock, 0, lines[i].debug, false);
                pushBlock(lambdaBlock, lambdaId, BlockType_e::lambda);
            } 
             
            // push a reference to the `for` iterator variable
            block.emplace_back(OpCode_e::PSHUSRVREF, rowVar, lines[i].debug);
            // push an iterator instruction (iterator instruction is essentially a marshal)
            block.emplace_back(
                reverseSearch
                    ? OpCode_e::ITRFORR
                    : OpCode_e::ITFORR,
                ...
                lines[i].debug,
                lambdaId);
            block.back().params = maxIterations;
        }
        else if (lines[i].parts[0] == "__row_continue_for" || lines[i].parts[0] == "__row_continue_for_count")
        {
            /* original format:
             *    for <rowVar> in rows:
             * modified format:
             *    __row_for <rowVar> __rows:
             */
            const auto isCount       = lines[i].parts[0] == "__row_continue_for_count";
            const auto maxIterations = isCount ? stoll(lines[i].parts[1]) : 99'999'999;
            const auto rowVar = lines[i].parts[isCount ? 2 : 1];
            const auto reverseSearch = lines[i].parts[isCount ? 3 : 2] == "__rev_rows";
        ...
        else if (lines[i].parts[0] == "__row_continue_from_for" || 
                 lines[i].parts[0] == "__row_continue_from_for_count")
        {
            /* original format:
             *    for <rowVar> in rows:
             * modified format:
             *    __row_for <rowVar> __rows:
             */
            const auto isCount       = lines[i].parts[0] == "__row_continue_from_for_count";
            const auto maxIterations = isCount ? stoll(lines[i].parts[1]) : 99'999'999;
            const auto fromVar       = lines[i].parts[isCount ? 2 : 1];
            const auto rowVar        = lines[i].parts[isCount ? 3 : 2];
            const auto reverseSearch = lines[i].parts[isCount ? 4 : 3] == "__rev_rows";
            // look for `if` and generate index hints
            auto conditions = tokenizeBlockForLogic(lines[i].parts, 3, lines[i].isConditional, lines[i].isFunction);
            // add the `row` variable and mark it as `isRowObject`, these
            // will be parsed differently than regular variables
            ...
            // inject fromVar
            if (isNumeric(fromVar))
            {
                block.emplace_back(OpCode_e::PSHLITINT, stoll(fromVar), lines[i].debug);
            }
            else
            {
                if (!isVar(vars.userVars, fromVar))
                {
                    auto tempVar = Variable_s { fromVar, fromVar, "" };
                    vars.userVars.emplace(fromVar, tempVar);
                }
                block.emplace_back(OpCode_e::PSHUSRVAR, fromVar, lines[i].debug);
            } 
            ...
            //    lines[i].parts[0] == "continue" && (lines[i].parts[1] == "for" || lines[i].parts[1] == "from")            
            //   ))
        else if (lines[i].parts[0] == "for")
        {
            LineParts left;
            ...
            if (isIn && isRows)
            {
                ...
                const auto isFrom = lines[i].parts[1] == "from";

                const auto fromVar = lines[i].parts[2];
                ...
                const auto isLimit = isNumeric(lines[i].parts[parseIndex]);
                const auto limit = isLimit ? stoll(lines[i].parts[parseIndex]) : 9999999;

                if (isLimit)
                    parseIndex += 1;

                const auto rowVar = lines[i].parts[parseIndex];               
                idx++;
                LineParts condition;
            ...
            parseConditions(right, block, 0, lines[i].debug, false); /* For iterators:
             *
            ...
            block.emplace_back(OpCode_e::ITFOR, lines[i].block, lines[i].debug);
        ...
        else if (lines[i].parts.size() >= 2 && lines[i].parts[1] == "[")
        {
            ...
            {
                MiddleOpList derefOps;
                derefEndIdx = parseConditions(lines[i].parts, derefOps, derefEndIdx + 1, lines[i].debug, false);
                // TODO - check for zero length
                derefCaptures.emplace_back(derefOps);
                ++derefEndIdx;
            }                          // now do everything after the equals sign
            MiddleOpList rightSideOps; // RIP Dict
            parseConditions(lines[i].parts, rightSideOps, derefEndIdx + 1, lines[i].debug, false);
            // insert the right side ops
            block.insert(
                block.end(),
                make_move_iterator(rightSideOps.begin()),
                make_move_iterator(rightSideOps.end())); // insert the deref ops
            ...
            for_each(
                ...
                [&block](auto item)
                {
                    block.insert(block.end(), make_move_iterator(item.begin()), make_move_iterator(item.end()));
            ...
            block.emplace_back(op, lines[i].parts[0], lines[i].debug);
            // POPUSROBJ uses params to store the depth of the dereferencing 
            block.back().params = derefCaptures.size();
            vars.userVars.emplace(lines[i].parts[0], Variable_s { lines[i].parts[0], "" });
        }
        else if (lines[i].parts.size() >= 2 && MathAssignmentOperators.count(lines[i].parts[1]))
        {
            MiddleOpList rightSideOps;
            parseConditions(lines[i].parts, rightSideOps, 2, lines[i].debug, false);
            block.insert(block.end(), make_move_iterator(rightSideOps.begin()), make_move_iterator(rightSideOps.end()));
            ...
            block.emplace_back(MathAssignmentOperators.find(lines[i].parts[1])->second, leftSide, lines[i].debug);
        }
        else if (lines[i].parts.size() >= 2 && (lines[i].parts[1] == "=" || lines[i].parts[1] == "<<"))
        {
            MiddleOpList rightSideOps;
            parseConditions(lines[i].parts, rightSideOps, 2, lines[i].debug, false);
            block.insert(block.end(), make_move_iterator(rightSideOps.begin()), make_move_iterator(rightSideOps.end()));
            ...
            {
                block.emplace_back(OpCode_e::POPRESCOL, lines[i].parts[0], lines[i].debug);
            }
            else
            {
                block.emplace_back(OpCode_e::POPUSRVAR, lines[i].parts[0], lines[i].debug);
                vars.userVars.emplace(lines[i].parts[0], Variable_s { lines[i].parts[0], "" });
            }
        }
        else if (Marshals.find(lines[i].parts[0]) != Marshals.end())
        {
            ...
            parseCall(lines[i].parts, functionCallOps, 0, lines[i].debug);
            block.insert(
                block.end(),
                make_move_iterator(functionCallOps.begin()),
                make_move_iterator(functionCallOps.end()));
        }
        else if (lines[i].parts.size() > 1 && (lines[i].parts[1] == "(" && lines[i].parts[0] != "if"))
        {
            MiddleOpList functionCallOps;
            parseCall(lines[i].parts, functionCallOps, 0, lines[i].debug);
            block.insert(
...
bool QueryParser::checkBrackets(LineParts& conditions)
{
    auto curly  = 0;
    auto square = 0;
    auto round  = 0;
...
int QueryParser::search(LineParts& conditions, const string& value, int startIdx)
{
    ...
    for (; startIdx < static_cast<int>(conditions.size()); ++startIdx)
        if (conditions[startIdx] == value)
            break;
    return (startIdx == static_cast<int>(conditions.size()))
               ? -1
               : startIdx;
}
...
QueryParser::LineParts QueryParser::extractVariable(LineParts& conditions, const int startIdx, int& reinsertIdx)
{
    ...
    auto index           = startIdx; // this variable is not dereferencing a container
    if (index + 1 < static_cast<int>(conditions.size()) && conditions.at(index + 1) != "[")
    {
    ...
    else
    {
        ...
        while (index < static_cast<int>(conditions.size())) // look for ending two brackets
        {
            ...
            if (conditions[index] == "]")
                --brackets;
            if ((brackets == 0 && index == static_cast<int>(conditions.size()) - 1) || (brackets == 0 && conditions[
                index + 1] != "["))
            {
                bracketComplete = true;
                break;
        ...
        if (bracketComplete)
        {
            result.insert(result.begin(), conditions.begin() + startIdx, conditions.begin() + index);
        }
    }
    if (!bracketComplete)
    {
    ...
    conditions.erase(conditions.begin() + startIdx, conditions.begin() + (startIdx + result.size()));
...
QueryParser::LineParts QueryParser::extractVariableReverse(LineParts& conditions, const int startIdx, int& reinsertIdx)
{
    ...
    else
    {
        ...
        while (index >= 0) // look for ending two brackets
        {
            ...
            if (conditions[index] == "]")
                ++brackets;
            if ((brackets == 0 && index == 0) || (brackets == 0 && conditions[index - 1] != "]"))
            {
                bracketComplete = true;
                break;
            }
        ...
        result.insert(result.begin(), conditions.begin() + index, conditions.begin() + startIdx + 1);
    }
    if (!bracketComplete)
    {
        reinsertIdx = -1; // error
    ...
    conditions.erase(conditions.begin() + index, conditions.begin() + startIdx + 1);
    reinsertIdx = index;
    return result;
}
...
void QueryParser::extractFunction(LineParts& conditions, const int startIdx, int& endIdx)
{
    auto bracketComplete = false;
    auto index           = startIdx; // this variable is not dereferencing a container
    if (index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] != "(")
    {
        endIdx = startIdx;
        return;
    }
...
void QueryParser::extractParam(LineParts& conditions, const int startIdx, int& endIdx)
{
    ...
    while (index < static_cast<int>(conditions.size())) // look for ending two brackets
    {
        if (conditions[index] == "(" || conditions[index] == "[")
            ++brackets;
        if (conditions[index] == ")" || conditions[index] == "]")
            --brackets; // found the bracket at the end of a function call?
        if (brackets == -1 && conditions[index] == ")")
        {
            brackets = 0;
            break;
        }
        if (brackets == 0 && conditions[index] == ",")
            break;
        ++index;
    }
    if (brackets != 0)
    ...
    while (true)
    {
        ...
        do // I never DO these
        {
            ...
            while (index < static_cast<int>(conditions.size()))
            {
                ...
                if (conditions[0] == "for" || (static_cast<int>(conditions.size()) > 2 && ((conditions[0] == "continue"
                    && conditions[1] == "for") || (conditions[0] == "continue" && conditions[1] == "from"))))
                {
                    auto inPos  = -1;
                    ...
                    if (inPos != -1 && static_cast<int>(conditions.size()) > inPos + 1 && conditions[inPos + 1] ==
                        "rows")
                    {
                        const auto& rowVar = conditions[inPos - 1]; // add the row var, and flag it as `isRowObject`
                        if (!isVar(vars.userVars, rowVar))
                        {
                            auto tempVar        = Variable_s { rowVar, rowVar, "" };
                            tempVar.isRowObject = true;
                ...
                // TRANSLATE FIRST ROW into FIRST_ROW
                if (conditions[index] == "FIRST" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "ROW")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "FIRST_ROW";
                    changes           = true; // loop again
                    break;
                } 

                // TRANSLATE LAST ROW into LAST_ROW
                if (conditions[index] == "LAST" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "ROW")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "LAST_ROW";
                    changes           = true; // loop again
                    break;
                } 

                // TRANSLATE FIRST VALUE into FIRST_VALUE
                if (conditions[index] == "FIRST" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "VALUE")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "FIRST_VALUE";
                    changes           = true; // loop again
                    break;
                } 

                // TRANSLATE LAST VALUE into LAST_VALUE
                if (conditions[index] == "LAST" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "VALUE")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "LAST_VALUE";
                    changes           = true; // loop again
                    break;
                } 
                
                // TRANSLATE COUNT DISTINCT into COUNT_DISTINCT
                if (conditions[index] == "COUNT" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "DISTINCT")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "COUNT_DISTINCT";
                    changes           = true; // loop again
                    break;
                } 
                ...
                if (conditions[index] == "not" && 
                    index + 1 < static_cast<int>(conditions.size()) && conditions[index + 1] == "in")
                {
                    conditions.erase(conditions.begin() + index + 1);
                ...
                // allow the SQL peeps to use `where` instead of `if`
                // for iterator and aggregator logic
                if (conditions[index] == "where")
                {
                    conditions[index] = "if";
                    changes = true;
                    break;
                 ...
                 *
                 */
                if (conditions[index] == "FIRST_ROW" ||
                    conditions[index] == "LAST_ROW" ||
                    conditions[index] == "FIRST_VALUE" ||
                    conditions[index] == "LAST_VALUE" || 
                    conditions[index] == "SUM" || 
                    ...
                    conditions[index] == "COUNT_DISTINCT")
                {
                    ...
                    // shameful hack - FIRST_ROW and LAST_ROW don't take a variable
                    // but the code below is expecting keywords in certain spaces, so
                    // we will inject a placeholder
                    if (agg == "FIRST_ROW" || agg == "LAST_ROW")
                           aggregate.insert(aggregate.begin() + 1, "stamp");

                    const auto variable = aggregate[1];

                    if (aggregate.size() <= 2 || 
                        (aggregate.size() > 2 && aggregate[2] != "if" && aggregate[2] != "where"))
                    {
                        // If no `where` we add one, this allows aggregation without the "where".
                        aggregate = LineParts { agg, variable, "if", variable, "!=", "None" };
                    }
                    else
                    {
                        aggregate.insert(aggregate.begin() + 3, "(");
                        aggregate.insert(aggregate.end(), { ")", "and", variable, "!=", "None" });
                    }

                    // make up a function name
                    ...
                    if (aggregate.size() > 2) // this is the condition
                        where.insert(where.begin(), aggregate.begin() + 2, aggregate.end());

                    const auto isSearch = (agg == "LAST_ROW" || agg == "LAST_VALUE" || agg == "FIRST_ROW" || agg == "FIRST_VALUE");
                    const auto reverseSearch = (agg == "LAST_ROW" || agg == "LAST_VALUE");
                    const auto maxIterations = isSearch ? "1" : "99999999"; 
                    ...
                    FirstPass newFunction = {
                        ...
                        {
                            // searches return None rather than 0 if they don't match
                            { "__agg_result", "=", isSearch ? "None" : "0" },
                            lastDebug,
                            1
                        }
                    };
                     
                    LineParts nextWhere = { "__row_for", maxIterations, "agg_row", reverseSearch ? "__rev_rows" : "__rows" };
                    nextWhere.insert(nextWhere.end(), where.begin(), where.end());
                    nextWhere.push_back("__MARKER__"); //if (!with.length())
                        ...
                        newFunction.insert(
                            newFunction.end(),
                            {
                                ...
                                },
                                {
                                    ...
                                    { "__agg_result", "=", "agg_row" },
                                    lastDebug,
                                    2
                                },
                            });
                    }
                    else if (agg == "FIRST_VALUE" || agg == "LAST_VALUE")
                    {
                        newFunction.insert(
                            newFunction.end(),
                            {
                 ...
                 */
                if (static_cast<int>(conditions.size()) > index + 1 && conditions[index] == "del")
                {
                    ...
                    newStatement.insert(newStatement.end(), conditions.begin() + 1, conditions.begin() + idx);
                    newStatement.push_back(",");
                    newStatement.push_back(key);
                    newStatement.push_back(")"); // make first param a reference
                    newStatement[2] = "@" + newStatement[2];
                    line.parts.swap(newStatement);
                    changes = true;
                    break;
                }
                if (static_cast<int>(conditions.size()) > index + 1 && conditions[index] == "{")
                {
                    const auto originalIndex = index;
                    auto brackets            = 0;
                    auto allCounted          = false;
                 ...
                 *      
                 */
                if (conditions[index] == "[" && isSplice(conditions, index))
                {
                    ...
                    if (inParts.back() == "__MARKER__")
                        inParts.push_back("None"); // replace the marker with a comma
                    inParts[1] = ",";
                    LineParts newSection;
                    newSection.push_back(command);
                    newSection.push_back("(");
                    newSection.push_back(containerRef); // if the original member had parameters 
                    if (inParts.size())
                    {
                        newSection.push_back(",");
                 ...
                 *
                 */
                if (basic_string<char>::size_type pos; (pos = conditions[index].find(".find")) != std::string::npos
                    || (
                        pos                                          = conditions[index].find(".rfind")) != std::string
                    ::npos || (pos                                   = conditions[index].
                        find(".split")) != std::string::npos || (pos = conditions[index].find(".strip")) != std::string
                    ::
                    npos || (pos                                   = conditions[index].find(".append")) != std::string
                    ::npos || (pos                                 = conditions[index].
                        find(".pop")) != std::string::npos || (pos = conditions[index].find(".clear")) != std::string
                    ::npos
                    || (pos                        = conditions[index].find(".keys")) != std::string::
                    npos || (pos                   = conditions[index].find(".add")
                    ) != std::string::npos || (pos = conditions[index].find(".remove")) != std::string
                    ::npos || (pos                 =
                        conditions[index].find(".update")) != std::string::npos)
                {
                 ...
                 *      for row __rows
                 */
                if (conditions[index] == "in" && 
                    index + 1 < static_cast<int>(conditions.size()) && 
                    conditions[index + 1] == "rows")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "__rows";
                    changes           = true; // loop again
                    break;
                }

                if (conditions[index] == "in" && 
                    index + 2 < static_cast<int>(conditions.size()) && 
                    conditions[index + 1] == "reverse" && conditions[index + 2] == "rows")
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "__rev_rows";
                    changes           = true; // loop again
                    break;
                } 
                 ...
                 *       __row_for <rowVar> __rows:
                 */
                if ((conditions[index] == "for" && 
                    (
                        (index + 2 < static_cast<int>(conditions.size()) && 
                            (conditions[index+ 2] == "__rows" || conditions[index + 2] == "__rev_rows")) || 
                        (index + 3 < static_cast<int>(conditions.size()) && 
                            (conditions[index + 3] == "__rows" || conditions[index + 3] =="__rev_rows"))
                    )))
                {
                    conditions[index] = "__row_for";
                    changes           = true; // loop again
                    break;
                }

                if (index == 0 && conditions[index] == "continue" && 
                    index + 1 < static_cast<int>(conditions.size()) &&
                    (conditions[index + 1] == "__row_for" || conditions[index + 1] == "for"))
                {
                    conditions.erase(conditions.begin() + index + 1);
                    conditions[index] = "__row_continue_for";
                    changes           = true; // loop again
                    break;
                } 
                 ...
                 *  convert:
                 *
                 *      continue from some_row __row_for row __rows:
                 *      
                 *  to
                 *  
                 *      __row_continue_from_for <fromVar> <rowVar> __rows:
                 *      
                 */
                if (index == 0 && index + 2 < static_cast<int>(conditions.size()) && 
                    conditions[index] == "continue" &&
                    conditions[index + 1] == "from" && 
                    ((index + 2 < static_cast<int>(conditions.size()) && conditions[index + 2] == "__row_for") || 
                    (index + 3 < static_cast<int>(conditions.size()) && conditions[index + 3] == "__row_for") || 
                        (index + 2 < static_cast<int>(conditions.size()) && conditions[index + 2] == "for") || 
                        (index + 3 < static_cast<int>(conditions.size()) && conditions[index + 3] == "for")))
                {
                    const auto fromVar = conditions[index + 2];
                    conditions.erase(conditions.begin() + index + 1);
                    if (conditions[index + 2] == "__row_for" || conditions[index + 2] == "for")
                        conditions.erase(conditions.begin() + index + 2);
                    else if (conditions[index + 3] == "__row_for" || conditions[index + 3] == "for")
                        conditions.erase(conditions.begin() + index + 3);
                    conditions[index]     = "__row_continue_from_for";
                    conditions[index + 1] = fromVar;
                    changes               = true; // loop again
                    break;
                }

                if (index == 0 && conditions[index] == "__row_for" && 
                    index + 1 < static_cast<int>(conditions.size()) &&
                    isNumeric(conditions[index + 1]))
                {
                    conditions[index] = "__row_for_count";
                    changes           = true; // loop again
                    break;
                }

                if (index == 0 && conditions[index] == "__row_continue_for" && 
                    index + 1 < static_cast<int>(conditions.
                    size()) && isNumeric(conditions[index + 1]))
                {
                    conditions[index] = "__row_continue_for_count";
                    changes           = true; // loop again
                    break;
                }

                if (index == 0 && conditions[index] == "__row_continue_from_for" && 
                    index + 2 < static_cast<int>(conditions.size()) && 
                    isNumeric(conditions[index + 2]))
                {
                    conditions[index] = "__row_continue_from_for_count";
                    std::swap(conditions[index + 1], conditions[index + 2]);
                    changes = true; // loop again
                    break;
                }
                ...
                if (conditions[index] == "break" && 
                    index + 1 < static_cast<int>(conditions.size()) && 
                    (conditions[index+ 1] == "all" || conditions[index + 1] == "top"))
                {
                ...
                if (index + 1 < static_cast<int>(conditions.size()) && TimeConstants.count(conditions[index + 1]))
                {
                    ...
                    const auto timeValue = TimeConstants.at(timeConst); // It's an optimizing compiler now!
                    if (isNumeric(item))
                    {
...
{
    FirstPass result; // make some space
    ...
    for (auto i = 0; i < static_cast<int>(lines.size()); i++)
    {
        ...
        auto& line = lines[i];
        lastDebug  = line.debug;               // these are easy, iterate lines until we find the marker
        if (parseMode == parseMode_e::query && // in query mode these are 
            line.parts.size() && line.parts.back() == "\\")
        {
            for (; i < static_cast<int>(lines.size()); i++)
            {
            ...
            (line.parts[0] == "if" || line.parts[0] == "elif" || line.parts[0] == "else" || line.parts[0] == "for" || (
                static_cast<int>(lines[i].parts.size()) > 2 && lines[i].parts[0] == "continue" && lines[i].parts[1] ==
                "for") || (static_cast<int>(lines[i].parts.size()) > 2 && lines[i].parts[0] == "continue" && lines[i].
                parts[1] == "from"))/* ||
                line.parts[0] == "match" ||
...
int64_t QueryParser::parseHintConditions(
    const LineParts& conditions,
    HintOpList& opList,
    int64_t index,
    const bool stopOnConditions)
{
    ...
    const auto store = [&]()
    {
        ...
        if (isColumnVar(left))
            left = vars.columnVars.find(left)->second.actual; //else if (isGroupVar(left))
...
void QueryParser::evaluateHints(const string& hintName, HintOpList& hintOps)
{
    ...
    parseHintConditions(
        (*hint).second,
        // the hints under this hint name
        hintOps,
        0,
        false);
}

void QueryParser::build(Columns* columns, MiddleBlockList& input, InstructionList& finCode, Variables_S& finVars)
{
    /*
    ...
    */ // index the select variables
    ...
    std::sort(
        finVars.columnVars.begin(),
        finVars.columnVars.end(),
    ...
    std::sort(
        finVars.tableVars.begin(),
        finVars.tableVars.end(),
        [](const Variable_s a, const Variable_s b)
        {
            return a.sortOrder < b.sortOrder;
        });
    ...
    // the index here.
    unordered_map<int64_t, int64_t> blockIndex; // this maps a function name to a specific index
    // in the final code
    unordered_map<string, int64_t> functionMap; // this records the function name and the position in the
    // final code, this is used post compile to map a function
    // name to an index
    unordered_map<int64_t, string> callMap; //for (auto blockFilter = 0; blockFilter < 2; ++blockFilter)
    {
        for (auto& b : input) // iterate the blocks
        {
            ...
            */ // first line of this block will be tail of
            ...
            blockIndex[b.blockId] = finCode.size(); // if this is a function map its name to the 
            // final Code line
            if (b.type == BlockType_e::function)
                functionMap[b.blockName] = finCode.size(); // iterate instruction
            for (auto& c : b.code)
            {
                switch (c.op)
                {
                ...
                case OpCode_e::PSHTBLCOL:
                    finCode.emplace_back(
                        ...
                        0,
                        c.deferredStr.length()
                            ? vars.userVars[c.deferredStr].index
                            : NONE,
                        c.debug);
                    break;
                ...
                case OpCode_e::VARIDX:
                    finCode.emplace_back(c.op, vars.userVars[c.valueString].index, 0, 0, c.debug);
                    break;
                ...
                case OpCode_e::PSHLITINT:
                    finCode.emplace_back(c.op, 0, c.value, 0, c.debug);
                    break;
                case OpCode_e::PSHLITFLT:
                    finCode.emplace_back(
                        ...
                        0,
                        c.value,
                        // TODO fix floats - which are ints, but different
                        0,
                        c.debug);
                    break;
                ...
                case OpCode_e::POPTBLCOL:
                    finCode.emplace_back(c.op, vars.tableVars[c.valueString].index, 0, 0, c.debug);
                    break;
                case OpCode_e::POPRESCOL:
                    finCode.emplace_back(c.op, vars.columnVars[c.valueString].index, 0, 0, c.debug);
                    break;
                case OpCode_e::CNDIF: case OpCode_e::CNDELIF:
                    finCode.emplace_back(
                        c.op,
                        c.value,
                        // value for the moment is block_id
                        0,
                        c.lambda,
    ...
    for (auto& f : finCode)
    {
        ...
        case OpCode_e::ITRFORRCF: // map index to index of BlockID
            ...
            break;
        case OpCode_e::CNDELSE: // map index to index of BlockID
...
bool QueryParser::compileQuery(const char* query, Columns* columnsPtr, Macro_s& macros, ParamVars* templateVars)
{
    ...
    {
        rawScript  = query;
        templating = templateVars; // we will be suing the schema to validate the
        // select and group and column. variables
        tableColumns = columnsPtr;
        vars.tableVars.emplace("stamp", Variable_s { "stamp", "grid" });
        vars.tableVars.emplace("event", Variable_s { "event", "grid" }); // breaks it out into lines
        auto firstPass = extractLines(query);
        BlockList blockList; // convert lines into blocks by indentation
        blockList.emplace_back(0, FirstPass {});
        extractBlocks(0, firstPass, blockList); // iterate through our list of blocks, converting line parts
        ...
        sort(
            middleBlocks.begin(),
            middleBlocks.end(),
            [](MiddleBlock_s& a, MiddleBlock_s& b)
            {
                return a.blockId < b.blockId;
            });
        if (useSessions && vars.tableVars.count("session") == 0)
            vars.tableVars.emplace("session", Variable_s { "session", "grid" });
    ...
    catch (ParseFail_s& caught)
    {
        ...
        error.set(caught.eClass, caught.eCode, caught.getMessage(), caught.getDetail());
        return false;
